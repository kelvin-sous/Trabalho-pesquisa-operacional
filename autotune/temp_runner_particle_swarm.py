# Auto-generated runner for Particle Swarm
import sys
import os
import json
import numpy as np
...
def main():
    ...
    with open(r"temp_optimization/config_particle_swarm.json", "r") as f:
        config = json.load(f)
    ...
    print("="*70)
    print(f"   OTIMIZAÇÃO: PARTICLE SWARM")
    print("="*70)
    print()
    
    # Configurações específicas por algoritmo
    if "Particle Swarm" == "Pattern Search":
    ...
    elif "Particle Swarm" == "Particle Swarm":
        optimizer = ParticleSwarm(
            objective_function=run_external_program,
            x0=config["x0"],
            n_particles=30,
    ...
    try:
        ...
        result_file = r"temp_optimization/config_particle_swarm.json".replace("config", "result")
        with open(result_file, "w") as f:
            json.dump(results, f, indent=2)
        
        print("\n" + "="*70)
        print(f"   PARTICLE SWARM - OTIMIZAÇÃO CONCLUÍDA")
        print("="*70)
        log(f"")
